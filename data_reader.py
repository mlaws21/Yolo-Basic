...
# def import_pretrain_data(data_path, W=112, batch_size=4, speed=20, is_grayscale=True):
#     training_data = []
#     # ytrain = []
...
def import_synth_data(data_path, W=112, batch_size=4, speed=20, is_greyscale=True):
    training_data = []
    ...
    for c in classes:
        for pre_image in os.listdir(os.path.join(data_path, c))[::speed]:
            
            img_path = os.path.join(data_path, c, pre_image)
            try:
                img = io.read_image(img_path, io.ImageReadMode.RGB)
            except:
                raise Exception(f"The following image is missing or corrupted: {img_path}")
            ...
            training_data.append((img, lab_converter[c]))#)torch.nn.functional.one_hot(tensor(lab), 2)))

        # print()
    image_dataset = ImageDataset(training_data)
    dl = DataLoader(image_dataset, batch_size=batch_size, sampler=RandomSampler(image_dataset))
    return dl
...
def show_image(img_path):
    ...
    image_array = img.squeeze().cpu().numpy()  # Assuming tensor is in ( H, W) format
